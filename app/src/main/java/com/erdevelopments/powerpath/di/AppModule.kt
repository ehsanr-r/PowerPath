...
object AppModule {
    ...
    fun provideDatabase(@ApplicationContext context: Context): PowerPathDatabase {
        return Room.databaseBuilder(
            ...
            // For now, during development, this avoids crashes when schema changes.
            // REMOVE this before release and add real migrations.
            .fallbackToDestructiveMigration()
            .build()
    }

    @Provides
    fun provideUserDao(db: PowerPathDatabase): UserDao = db.userDao()

    @Provides
    fun provideDayDao(db: PowerPathDatabase): DayDao = db.dayDao()

    @Provides
    fun providePlanDao(db: PowerPathDatabase): PlanDao = db.planDao()

    @Provides
    fun provideWorkoutDao(db: PowerPathDatabase): WorkoutDao = db.workoutDao()

    @Provides
    fun providePlanWorkoutDao(db: PowerPathDatabase): PlanWorkoutDao = db.planWorkoutDao()
}